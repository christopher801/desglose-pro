...
const buildPrintHtml = (projectInfo, results) => {
  ...
  const rows = results.map(row => `
    <tr>
      <td>${row.hueco}</td><td>${row.ancho}</td><td>${row.alto}</td><td>${row.tipo}</td>
      <td>${row.cabAlf}</td><td>${row.jambas}</td><td>${row.marco}</td><td>${row.latMarco}</td>
      <td>${row.vidrioAncho}</td><td>${row.vidrioAlto}</td>
    </tr>`).join('')
  return `<!DOCTYPE html><html><head><title>PUERTA COMERCIAL</title>
  ...
  <body><h1>PUERTA COMERCIAL TRADICIONAL</h1><h2>SISTEMA PROFESIONAL DE CÁLCULO</h2>
  <div class="info">
  ...
  <table><thead>
    <tr>
      <th rowspan="2">Hueco</th><th rowspan="2">Ancho</th><th rowspan="2">Alto</th><th rowspan="2">Tipo</th>
      <th colspan="2">Hoja</th><th colspan="2">Marco</th><th colspan="2">Vidrio</th>
    </tr>
    <tr><th>Cab-alf</th><th>Jambas</th><th>Cab-marco</th><th>Lat-marco</th><th>Ancho</th><th>Alto</th></tr>
    ...
    <Layout>
      <div className="page-content">
        <div className="desglose-header">
          ...
          <h1 className="page-title">Puerta Comercial</h1>
        ...
        <div className="card-modern mb-4">
          <div className="form-grid-4">
            ...
            <div className="auth-field">
              ...
              <input type="text" name="ancho" value={form.ancho} onChange={handleFormChange} placeholder='ej: 56 7/8"' className="auth-input" />
            </div>
            <div className="auth-field">
              <label className="auth-label">Alto</label>
              <input type="text" name="alto" value={form.alto} onChange={handleFormChange} placeholder='ej: 82 2/16"' className="auth-input" />
            </div>
            <div className="auth-field">
              <label className="auth-label">Tipo puerta</label>
            ...
            <div className="table-responsive">
              <table className="table-professional" style={{ minWidth: '820px' }}>
                <thead>
                  ...
                  <tr>
                    <th>Cab-alf</th><th>Jambas</th><th>Cab-marco</th><th>Lat-marco</th><th>Ancho</th><th>Alto</th>
                  </tr>
                </thead>
                <tbody>
                  {results.map((row, idx) => (
